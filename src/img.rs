...
/// Load all JPEG images from a directory, resize to fit max_dim x max_dim.
pub fn load_all(img_dir: &str, max_dim: u32) -> Result<Vec<LoadedImage>> {
    ...
    for entry in std::fs::read_dir(dir).context("reading img directory")? {
        ...
        // Resize to fit max_dim while maintaining aspect ratio
        ...
        // Extract normalized pixels [f64; 3]
        ...
        // Generate base64 thumbnail (200px max) for HTML preview
...
/// Compute new dimensions that fit within max_dim while preserving aspect ratio.
...
/// Resize image to fit preview_dim for HTML embedding.
...
/// Encode a DynamicImage as a base64 data URI string.
