...
fn write_algo_section(html: &mut String, name: &str, desc: &str, result: &ClusterResult4D) {
    ...
    // Dominant colour — use already-computed dominant_score (p × c)
    ...
    html.push_str(&format!(
        "<span class=\"dominant-proportion\">{:.1}% of pixels | L* {:.1} | a* {:.1} b* {:.1} | C {:.1} | c={:.3}{}</span>\n",
        result.dominant.proportion * 100.0,
        result.dominant.lab_l,
        result.dominant.lab_a,
        result.dominant.lab_b,
        result.dominant.chroma,
    ...
    html.push_str(&format!(
        "<span class=\"dominant-proportion\" style=\"font-size:0.7rem;color:#888;\">B={:.3} | edge={:.2} con={:.2} spread={:.3} | U={:.3} cw={:.3}</span>\n",
        result.dominant.backgroundness,
        result.dominant.edge_ratio,
        result.dominant.border_connected_ratio,
        result.dominant.spread,
        result.dominant.uniqueness,
        result.dominant.centre_weight,
    ));
    html.push_str("</div>\n");
    html.push_str("</div>\n");
    ...
    for cluster in &result.clusters {
        ...
        // Chroma badge
        html.push_str(&format!(
            "<span class=\"complexity-badge\" style=\"background:rgba(0,180,216,0.12);color:#0bd;\">C={:.1}</span>\n",
            cluster.chroma
        ));
        ...
        let bg = cluster.backgroundness;
        if bg > 0.01 {
            html.push_str(&format!(
                "<span class=\"complexity-badge\" style=\"background:rgba(255,80,80,0.12);color:#f66;\">B={:.2}</span>\n",
                bg
            ));
        }
        html.push_str("</div>\n");
...
fn write_footer(html: &mut String, timestamp: &str) {
    ...
    html.push_str(&format!("<p>Saved at: {timestamp}</p>\n"));
    html.push_str("</footer>\n");
    html.push_str("</body>\n</html>\n");
}
